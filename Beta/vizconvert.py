#!python3
import argparse, sys, os, sqlite3, csv
from datetime import datetime, timedelta
from unidecode import unidecode
# os, sys, subprocess, csv, time, datetime, re, multiprocessing

parser = argparse.ArgumentParser(description='Convert file from log2timeline to timeflow format')
parser.add_argument('src_location',nargs=1,help='Source File: Y:\\Reports\\sample.log')
parser.add_argument('dst_location',nargs='?',default='Visability',help='Destination Folder location. If nothing is supplied then the default is \'Visability\'')
...
# Set source location/file
	if not os.path.exists(args.src_location[0]):
		print("ERROR: \""+args.src_location[0]+"\" cannot be found by the system.  Please verify file location.")
		print("Exiting...")
		sys.exit(1)
...
# Read input file
fname = src_loc.split('\\')[-1]+".time"
fo = open(fname, 'w', errors='ignore')
infile = open(src_loc, 'rU', errors='ignore')

# Required to open some bad files
csv.field_size_limit(sys.maxsize)

# Create the header
header = ['date', 'timezone', 'MACB', 'source', 'sourcetype', 'type', 'user', 'host', 'short', 'desc', 'version', 'filename', 'inode', 'notes', 'format']
...
#for item in csv.reader((line.replace('\0','*') for line in infile), delimiter=","):

for line in infile:
	nline = unidecode(line.replace('\0','*'))
	dirtyline = nline.split(',')[0:16]
	if dirtyline[0] == "date":
		continue
	if dirtyline[0][0] == "#":
		continue

	# Remove tabs from each item
	cleanline = []
	for item in dirtyline:
		cleanline.append(item.replace('\t',' '))


	# Get date/time
	cleanline[0] = cleanline[0] + " " + cleanline[1]
	start_date = datetime.strptime(cleanline[0],'%m/%d/%Y %H:%M:%S')
	cleanline[0] = start_date.strftime('%b %d %Y %H:%M:%S')
	del cleanline[1]
